...
df_weekly_grouped_custom = df_weekly.groupby(['单品名称', '周数'])['销量(千克)'].sum().reset_index()

# 数据预处理：将每个品类的每周销量数据转换为矩阵形式
pivot_df = df_weekly_grouped_custom.pivot(index='单品名称', columns='周数', values='销量(千克)').fillna(0)
...
dendrogram(Z, labels=pivot_df.index, leaf_rotation=0, orientation='left')
plt.xlabel('单品名称')
plt.ylabel('欧氏距离')
plt.tight_layout()

plt.savefig('dendrogram_high_res.png', dpi=300, bbox_inches='tight')
plt.show()